...
- `ruby scripts/init/init_artifact.rb design_seed path/to/design_seed.yaml`
- `ruby scripts/init/init_artifact.rb --step-id init-06 design_seed runs/demo/init-06.design_seed.yaml`
- `ruby scripts/init/prefill_from_upstream.rb --step-id init-06 design_seed path/to/baseline.yaml path/to/design_seed.yaml`
...
- 虽然 `init-06` 不单独停给人，但进入 `init-07` 人工确认前，仍应把 `design_seed` 渲染成 Markdown，和 `bootstrap_plan` 一起给人通读

### 第五步：生成 bootstrap_plan

`init-07` 用于确定哪些内容属于“无需业务 PRD、但应该优先初始化”的项目底座计划。

- `ruby scripts/init/init_artifact.rb bootstrap_plan path/to/bootstrap_plan.yaml`
- `ruby scripts/init/init_artifact.rb --step-id init-07 bootstrap_plan runs/demo/init-07.bootstrap_plan.yaml`
- `ruby scripts/init/prefill_from_upstream.rb --step-id init-07 bootstrap_plan path/to/design_seed.yaml path/to/bootstrap_plan.yaml`
...
- 把干净版 `Project Conventions` 写入新项目
- 生成 run 内专用 `prompts/init-08-execution-prompt.md`
- 由执行代理完成工程初始化命令和 AI 补强
- 完成后自动执行 `post_init_to_prd.rb`
- 创建新的 `prd` run
- 注入干净版 `PRD Bootstrap Context`
- 预填新的 `raw/request.md`
- 生成新的 PRD 启动提示词，并明确引用规则文档

推荐脚本链路：

- `ruby scripts/init/render_init_execution_scope.rb path/to/bootstrap_plan.yaml runs/demo/rendered/init-08.init-execution-scope.md`
- `ruby scripts/init/execute_init_scope.rb path/to/bootstrap_plan.yaml --project-name <name>`
- 上一步会同时生成 `prompts/init-08-execution-prompt.md`
- 把 `prompts/init-08-execution-prompt.md` 交给执行代理
- 执行代理在工程初始化完成后执行 `ruby scripts/init/post_init_to_prd.rb path/to/bootstrap_plan.yaml --project-name <name>`
