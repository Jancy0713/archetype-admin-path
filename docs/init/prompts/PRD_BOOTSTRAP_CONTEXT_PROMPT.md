...
8. 输出必须严格遵循 `bootstrap_plan.template.yaml` 的字段名，只修改 `prd_bootstrap_context` 内容。

输出要求：

1. `prd_bootstrap_context.generation_workflow` 要体现“脚本初稿 + AI 补强 + reviewer 检查”的模式。
2. `priority_modules` 至少覆盖：
   - 登录与认证基础
   - 账号与成员基础
   - 框架型组件基础
   - 平台通用能力组件基础
   - 租户上下文基础
   - 权限与访问控制基础
3. 每个模块都应写清 `objective` 和一组可执行的 `requirements`。
4. `reviewer_focus` 要能帮助 reviewer 判断：
   - 是否还是基础 PRD
   - 是否遗漏基础组件或状态边界
   - 是否混入具体业务前提
