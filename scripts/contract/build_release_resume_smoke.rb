...
RUNS_ROOT = File.join("/tmp", "contract-build-release-resume-smoke")
...
begin
  ...
  state["next_action"] = "build_release"
  state["next_command"] = ["ruby", "scripts/contract/build_release.rb", run_root]
  write_yaml(state_path, state)

  FileUtils.rm_rf(File.join(run_root, "contract/release"))
  run!("ruby", "scripts/contract/build_release.rb", run_root)

  state = load_yaml(state_path)
  assert(state["status"] == "passed", "Direct build_release should seal review_result as passed")
  assert(state["next_action"] == "release_ready", "Direct build_release should update next_action to release_ready")
  assert(state["next_command"].nil?, "Direct build_release should clear next_command after success")

  puts "Build release resume smoke passed."
ensure
  FileUtils.rm_rf(RUNS_ROOT)
end
