...
RUNS_ROOT = File.join("/tmp", "contract-build-release-guard-smoke")
...
def write_review_result_snapshot(run_root, review_path, spec_path)
  payload = {
    "flow_id" => "contract",
    "step_id" => "contract-04",
    "status" => "releasing",
    "batch_id" => "batch-foundation-access",
    "contract_id" => "batch-foundation-access",
    "run_id" => File.basename(run_root),
    "review_path" => review_path,
    "subject_path" => spec_path,
    "review_sha256" => Digest::SHA256.file(review_path).hexdigest,
    "subject_sha256" => Digest::SHA256.file(spec_path).hexdigest,
  }
  write_yaml(File.join(run_root, "contract/working/state/contract-04.review-result.yaml"), payload)
end
...
begin
  ...
  write_review_result_snapshot(run_root, review_path, spec_path)

  spec = load_yaml(spec_path)
  spec["spec_scope"]["summary"] = "Mutated after review"
  write_yaml(spec_path, spec)

  run_expect_failure!("ruby", "scripts/contract/build_release.rb", run_root)
  assert(!File.exist?(File.join(run_root, "contract/release/openapi.yaml")), "Guarded build_release should not produce release outputs for mismatched flow binding")

  puts "Build release guard smoke passed."
ensure
  FileUtils.rm_rf(RUNS_ROOT)
end
