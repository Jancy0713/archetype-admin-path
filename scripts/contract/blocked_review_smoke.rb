...
RUNS_ROOT = File.join("/tmp", "contract-blocked-review-smoke")
...
def block_review(path, run_root)
  data = load_yaml(path)
  ...
  data["decision"]["has_blocking_issue"] = true
  data["decision"]["allow_release"] = false
  data["decision"]["need_human_escalation"] = false
  data["decision"]["suggested_return_step"] = "domain_mapping"
  data["decision"]["reason"] = "domain mapping needs revision"
  data["findings"]["p0"] = ["resource mapping is incomplete"]
  write_yaml(path, data)
end
...
begin
  ...
  block_review(review_path, run_root)
  run_expect_failure!("ruby", "scripts/contract/review_complete.rb", run_root, review_path)

  assert(!File.exist?(File.join(run_root, "contract/release/openapi.yaml")), "Release output should not exist after blocked review")

  progress = File.read(File.join(run_root, "progress/workflow-progress.md"))
  assert(progress.include?("current_step_id: contract-02"), "Progress board did not return to the suggested step")
  assert(progress.include?("overall_status: blocked"), "Progress board did not remain blocked")
  assert(progress.include?("next_agent_input: contract/working/contract-02.domain_mapping.yaml"), "Progress board did not point back to domain_mapping")
  assert(progress.include?("| `contract-03` | `todo` |"), "Progress board left contract-03 in review after returning to domain_mapping")

  puts "Blocked review smoke passed."
ensure
  FileUtils.rm_rf(RUNS_ROOT)
end
