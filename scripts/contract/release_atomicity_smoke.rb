...
RUNS_ROOT = File.join("/tmp", "contract-release-atomicity-smoke")
...
begin
  ...
  original_spec_content = File.read(spec_path)
  spec["api_surface"]["endpoints"] = [] # Trigger failure in build_release
  write_yaml(spec_path, spec)

  # Note: review_complete.rb will fail because it calls build_release internally
  run_expect_failure!("ruby", "scripts/contract/review_complete.rb", run_root, review_path)
  assert(!File.exist?(File.join(run_root, "contract/release/openapi.yaml")), "Release output should not exist after failed release build")
  state_path = File.join(run_root, "contract/working/state/contract-04.review-result.yaml")
  if File.exist?(state_path)
    state = load_yaml(state_path)
    assert(state["status"] == "releasing", "Failed release build should not mark review result as passed")
  end

  # Restore and retry
  File.write(spec_path, original_spec_content)
  run!("ruby", "scripts/contract/review_complete.rb", run_root, review_path)
  assert(File.exist?(File.join(run_root, "contract/release/openapi.yaml")), "Release output should exist after successful retry")

  puts "Release atomicity smoke passed."
ensure
  FileUtils.rm_rf(RUNS_ROOT)
end
