...
def usage
  warn "Usage: ruby scripts/init/profile/init_project_profile_step.rb <run_dir> <init-01|init-02|init-03|init-04> [--force]"
  exit 1
end
...
target = File.join(run_root, "init/#{step_id}.project_profile.yaml")
command = ["ruby", "scripts/init/init_artifact.rb"]
command << "--force" if force
command += ["--step-id", step_id, "project_profile", target]
