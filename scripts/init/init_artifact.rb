...
if artifact.nil? || target.nil?
  warn "Usage: ruby scripts/init/init_artifact.rb [--force] [--step project_initialization] [--step-id init-01] [--flow-id init] [--artifact-id init-01.project_profile] <project_profile|review|baseline|change_request> <target.yml>"
  exit 1
end
...
content = InitFlow::ArtifactUtils.load_yaml(template)
...
if artifact == "review" && step
  unless InitFlow::ArtifactUtils::REVIEWABLE_SUBJECTS.key?(step)
    warn "Invalid review step: #{step}"
    exit 1
  end
  content["status"]["step"] = step
  content["meta"]["subject_type"] = InitFlow::ArtifactUtils::REVIEWABLE_SUBJECTS.fetch(step)
end
